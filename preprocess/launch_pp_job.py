...
if __name__ == "__main__":
    ...
    current_list = []
    for i, name in enumerate(sim_list):
        ### charlie's code
        job_name = "%s-%s" % (params['kill_tag'], str(name).zfill(4))
        ...
        current_list.append(job_name)
        if (i + 1) % 5 == 0:
            
            print(f"i={i},sleeping...")
            time.sleep(seconds)
            for job in current_list:
                subprocess.run(["kubectl", "delete", "job", job])     
                time.sleep(5)
            print("resetting current list...")
            current_list = []
            time.sleep(30)
            print(f"current list should be empty: {current_list}")
